...
def ui(result)
  ...
  puts
  puts '==== Number of unique different entities =================='
  puts "services:\t #{result[:unique_services]}"
  puts "applications:\t #{result[:unique_apps]}"
  puts
  puts '==== Number of different service_id\'s used by provider Stats =================='
  result[:service_by_provider_cdf].sort_by { |key, _| key }.each do |perc, val|
    puts "Percentile #{perc * 100}: #{format('%.2f', val)}"
  end
  puts
  puts '==== Number of different app_id\'s used by provider Stats =================='
  result[:app_by_provider_cdf].sort_by { |key, _| key }.each do |perc, val|
    puts "Percentile #{perc * 100}: #{format('%.2f', val)}"
  end
  puts
